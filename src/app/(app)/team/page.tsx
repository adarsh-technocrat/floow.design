...
export default function TeamPage() {
  ...
  useEffect(() => {
    ...
    dispatch(fetchPendingInvitesForUser());
  }, [user?.uid, dispatch]);

  // Auto-select first team when teams load
  const resolvedTeamId =
    selectedTeamId ?? (teams.length > 0 ? teams[0].id : null);
  if (resolvedTeamId && resolvedTeamId !== selectedTeamId) {
    setSelectedTeamId(resolvedTeamId);
  }

  useEffect(() => {
    if (!selectedTeamId || !user?.uid) return;
    dispatch(fetchTeamDetail({ teamId: selectedTeamId }));
    dispatch(fetchTeamInvites({ teamId: selectedTeamId }));
  ...
  const handleRespondToInvite = useCallback(
    async (token: string, action: "accept" | "decline") => {
      if (!user?.uid) return;
      await dispatch(respondToTeamInvite({ token, action })).unwrap();
      dispatch(fetchUserTeams());
    },
    [user?.uid, dispatch],
  );
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-t-tertiary mt-0.5">
                        Invited by{" "}
                        {invite.inviter.displayName || invite.inviter.email} as{" "}
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          handleRespondToInvite(invite.token, "accept")
                        }
                        className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-emerald-500 px-3 text-xs font-medium text-white hover:bg-emerald-600"
                      >
                        <Check className="size-3.5" /> Accept
                      </button>
                      <button
                        onClick={() =>
                          handleRespondToInvite(invite.token, "decline")
                        }
                        className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-b-secondary px-3 text-xs font-medium text-t-secondary hover:bg-input-bg"
                      >
            ...
            {teamsLoading ? (
              ...
              <div className="rounded-xl border border-b-secondary bg-surface-elevated p-8 text-center">
                ...
                <button
                  ...
                  className="mt-4 inline-flex h-9 items-center gap-2 rounded-lg bg-btn-primary-bg px-4 text-xs font-semibold text-btn-primary-text hover:opacity-90"
                >
              ...
              <div className="flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  {teams.length > 1 ? (
                    <select
                      ...
                      className="rounded-lg border border-b-secondary bg-input-bg px-3 py-2 text-sm text-t-primary outline-none"
                    >
                      {teams.map((t) => (
                ...
                {showCreateForm && (
                  <div className="rounded-xl border border-b-secondary bg-surface-elevated p-4">
                    <label className="mb-1.5 block text-[11px] font-mono uppercase tracking-wider text-t-tertiary">
                      Team name
                    </label>
                    <input
                      type="text"
                      ...
                      placeholder="My Team"
                      className="w-full rounded-lg border border-b-secondary bg-input-bg px-3 py-2 text-sm text-t-primary placeholder:text-t-tertiary outline-none focus:border-t-secondary"
                      autoFocus
                      onKeyDown={(e) => {
                    ...
                    <div className="mt-3 flex justify-end gap-2">
                      <button
                        ...
                        className="rounded-lg px-3 py-1.5 text-xs text-t-secondary hover:bg-input-bg"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        className="rounded-lg bg-btn-primary-bg px-3 py-1.5 text-xs font-medium text-btn-primary-text disabled:opacity-50"
                      >
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          ...
                          <p className="text-xl font-semibold text-t-primary font-mono">
                            {activeTeam.credits.toLocaleString()}
                          </p>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-lg font-semibold text-t-primary">
                            {activeTeam.members.length} / {activeTeam.seats}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="rounded-xl border border-b-secondary bg-surface-elevated p-6">
                      ...
                      <div className="space-y-1">
                        {activeTeam.members.map((member) => (
                          <div
                            key={member.id}
                            className="group flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-input-bg/50"
                          >
                            <Avatar
                            ...
                            <span
                              className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-mono font-medium ${member.role === "OWNER" ? "bg-amber-500/15 text-amber-600 dark:text-amber-400" : member.role === "ADMIN" ? "bg-blue-500/15 text-blue-600 dark:text-blue-400" : "bg-surface-sunken text-t-tertiary"}`}
                            >
                              {member.role === "OWNER" && (
                                ...
                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  {currentUserRole === "OWNER" && (
                                    <button
                                      ...
                                      className="rounded p-1 text-t-tertiary hover:bg-input-bg hover:text-t-primary"
                                      title={
                                        member.role === "ADMIN"
                                          ? "Demote to member"
                                          : "Promote to admin"
                                  ...
                                  <button
                                    ...
                                    className="rounded p-1 text-t-tertiary hover:bg-red-500/10 hover:text-red-500"
                                    title="Remove member"
                                  >
                      ...
                      <div className="rounded-xl border border-b-secondary bg-surface-elevated p-6">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 relative">
                            ...
                            <input
                              ...
                              className="w-full rounded-lg border border-b-secondary bg-input-bg pl-9 pr-3 py-2 text-sm text-t-primary placeholder:text-t-tertiary outline-none focus:border-t-secondary"
                              onKeyDown={(e) => {
                          ...
                          <select
                            ...
                            className="rounded-lg border border-b-secondary bg-input-bg px-3 py-2 text-xs font-mono text-t-secondary outline-none"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-btn-primary-bg px-4 text-xs font-semibold text-btn-primary-text hover:opacity-90 disabled:opacity-50"
                          >
                        ...
                        {invites.length > 0 && (
                          <div className="mt-4 space-y-1">
                            <p className="text-[11px] font-mono uppercase tracking-wider text-t-tertiary mb-2">
                              Pending Invites
                            </p>
                            {invites.map((inv) => (
                              <div
                                key={inv.id}
                                className="flex items-center justify-between rounded-lg px-3 py-2 hover:bg-input-bg/50"
                              >
                                <div>
                                  <p className="text-[13px] text-t-primary">
                                    {inv.email}
                                  </p>
                                  <p className="text-[10px] text-t-tertiary">
                                    as {inv.role.toLowerCase()} · expires{" "}
                                ...
                                <button
                                  onClick={() =>
                                    dispatch({
                                      type: "team/revokeInvite",
                                      payload: inv.id,
                                    })
                                  }
                                  className="text-xs text-t-tertiary hover:text-red-500"
                                >
                                  Revoke
                                </button>
                              </div>
