...
export function PricingDialog({ open, onClose, reason }: PricingDialogProps) {
  ...
  const resetDate = isExhausted
    ? formatResetDate(userPlan?.creditsResetAt ?? null)
    : null;
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent
        ...
        className="flex max-h-[90vh] min-h-0 flex-col overflow-hidden p-0 sm:max-w-5xl"
      >
              ...
              <p className="mt-2 text-xs text-t-tertiary">
                ...
                <span className="font-medium text-t-secondary">
                  {currentPlanName.charAt(0) +
                    currentPlanName.slice(1).toLowerCase()}
                </span>
                {userPlan && (
                  <>
                    {" "}
                    &middot; {userPlan.credits}/{userPlan.creditCap} credits
                    remaining
                  </>
                )}
              </p>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-b-secondary bg-input-bg/30">
            <p className="text-[11px] text-t-tertiary">
              All plans include unlimited design & Figma exports. Cancel
              anytime.
            </p>
            <button
              onClick={() => {
